...
/// Moves a whole course section up and down within the course
    ...
    if (!$lock = $lockfactory->get_lock('courseid_' . $course->id, 60)) {
        throw new \moodle_exception('locktimeout', '', '', null, 'core_section_moveto: ' . $course->id);
    }

    try {

        // Get all sections for this course and re-order them (2 of them should now share the same section number).
        if (!$sections = $DB->get_records_menu('course_sections', ['course' => $course->id],
        ...
        // Adjust the highlighted section location if we move something over it either direction.
